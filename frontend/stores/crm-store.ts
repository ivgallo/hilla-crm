...
export class CrmStore {
    ...
    async editContact(contact: Contact) {
        ...
        try {
            const editedContact = await CrmEndpoint.editContact(contact.id) as Contact;
            this.saveLocal(editedContact);
            runInAction( () => {
                listViewStore.selectedContact = editedContact;
            })

        } catch (e) {
            uiStore.showError('Failed to edit contact.');
        }
    }
